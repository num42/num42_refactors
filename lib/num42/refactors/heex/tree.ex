defmodule Num42.Refactors.Heex.Tree do
  @moduledoc """
  ...
  # Walk forward from the start of `node`'s line and find the byte
  # offset (within that line) of the marker character that opens the
  # node: `<` for elements, `{` for eex_expr, `<%` for eex_block.
  ...
  # Returns `{:open, pos}` (just past `>`) or `{:self, pos}` (just past
  # `/>`) — the byte offset right after the opening tag's terminator.
  ...
  # the block-open. Initial depth is 0; we increment when we see the
  # `do %>` of the opener, then balance opens/closes thereafter.
  ...
  # --- Sigil collection (mirrors ExtractHeexFor.collect_h_sigils/1) ---
  ...
  # Tokenize HTML inside a `:text` chunk: split into elements, attribute
  # interpolations, and plain text. We do this with a small char-by-char
  # tokenizer rather than pulling in a full HTML parser — HEEx doesn't
  # need DOCTYPE/CDATA/scripts handled, and the goal is structural
  # comparison, not strict validity.

  # The scanner threads a current line counter through every byte it
  # consumes. The original EEx `:text` token only gives the line at
  # which the chunk *starts*; HTML elements and curlies inside the
  # chunk can sit many lines below. Without bumping `line` on each
  # `\n` we'd report every nested clone at its enclosing sigil's first
  # line — which made the cluster output land on `~H"""` openings.
  ...
  # Build a node list from the unified event stream (HTML tokens +
  # EEx events). Self-closing tags emit one `:element`; open/close
  # pairs nest. EEx blocks form their own frame on the same stack so
  # an HTML element can span them. Stray closes are tolerated.
  ...
  # Split a literal text chunk on `{…}` interpolations. Each emitted
  # node carries the line at which its first character begins —
  # crucial for clones: a `<.foo />` four lines below its enclosing
  # text token must report the correct file line, not the token's
  # start line.
