defmodule Num42.Refactors.Refactors.ExtractHeexExactClone do
  @moduledoc """
  ...
  def transform(source, opts) do
    with module when is_binary(module) <- core_components_module(opts),
         %{plans: plans} = prepared <- opts[:prepared] do
      cond do
        ...
        # plumbing.
        plans != [] and core_components_source?(source, module) ->
          append_components(source, plans)

        target_file = resolve_target_file(source, opts, prepared) ->
          apply_plans_to_file(source, plans, target_file)
  ...
  # `bound` = pattern names introduced by an enclosing `<%= for %>`.
  # We treat references that match a `bound` name as *locals* and
  # remove them from `assigns` even if they were also seen with `@`.
    ...
    {_, {a, l}} =
      Macro.prewalk(ast, {a, l}, fn
        ...
        {name, _, ctx} = node, {a, l} when is_atom(name) and is_atom(ctx) ->
          ...
          cond do
            String.starts_with?(str, "_") -> {node, {a, l}}
  ...
  # Try to parse an EEx-block header as `for pattern <- coll do`.
  # Returns `{:ok, pattern_var_names, collection_assign_names}`
  # or `:error` for non-`for` blocks.
  ...
  # Rewrite bare references to loop-binding names as `@name`. We
  # restrict to word boundaries that are not already preceded by `@`
  # or another word char, so `house.name` doesn't become `@house@.name`.
