defmodule Num42.Refactors.Refactors.ExtractInlineBlock do
  ...
  defp arg_names_of({kind, _, [head | _]}) when kind in [:def, :defp] do
    strip_when(head) |> handle_strip_when_3()
  end
  ...
  defp clause_callsite_patch(clause, helper_args) do
    ...
    Sourceror.get_range(clause) |> handle_get_range(replacement)
  end
  ...
  # FIXME: extracted automatically by ExtractCaseToHelper — review
  # the parameter list and consider a better name.
  defp handle_strip_when_3({_, _, args}) when is_list(args) do
    args
    |> Enum.flat_map(fn
  ...
  defp handle_get_range(_, _replacement), do: nil
end
