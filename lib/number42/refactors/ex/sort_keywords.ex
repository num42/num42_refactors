defmodule Number42.Refactors.Ex.SortKeywords do
  ...
  where reordering is safe under normal Elixir semantics:
  ...
  order-sensitive — `Ecto.Query.from(.., where: .., order_by: ..,
  select: ..)`, `import X, only: [a: 1, b: 2]`, plug pipelines,
  `case do ... end` clauses, `attr` / `slot` macro calls in Phoenix
  components — and a blanket sort would silently break behaviour. We
  stick to the narrow set where order is clearly irrelevant; even
  there, no rewrite is formally proven, so review the diff.
  ...
  def explanation do
    ...
    touches shapes where the runtime is expected to be indifferent to
    key order — `defstruct`, map and struct literals — and skips the
    order-sensitive cases (Ecto query parts, `import only:`, etc.).
    Skipping is the safety mechanism; there is no formal proof, so
    review the diff.
    """
  end
